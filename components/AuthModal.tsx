      ...
      <div className="nb-card w-full max-w-[420px] p-8 bg-paper relative">
        ...
        {mode === "done" ? (
          <div className="text-center py-4">
            <div className="w-14 h-14 nb-step mx-auto mb-5" style={{ borderRadius: "14px", fontSize: "22px" }}>
              ✉️
            </div>
            ...
            <p className="text-[14px] text-muted leading-[1.85]" style={{ wordBreak: "keep-all" }}>
              <strong className="text-navy-800">{email}</strong>으로<br />
              인증 링크를 보냈습니다.<br />
              메일함을 확인하고 링크를 클릭해 가입을 완료해주세요.
            </p>
            <button onClick={onClose} className="nb-btn w-full py-3.5 rounded-xl font-black text-[15px] mt-6">
              확인
            </button>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && (
                <p className="text-[13px] font-semibold text-red-500 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5">
                  {error}
                </p>
              )}
